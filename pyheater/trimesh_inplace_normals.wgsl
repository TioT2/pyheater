...
fn vs_main(d: VSData) -> FSData {
    let position = env.world_view_projection * vec4(d.position, 1.0);
    return FSData(
        position,
...
fn fs_main(d: FSData) -> @location(0) vec4<f32> {
    let normal = normalize(cross(dpdx(d.pos), dpdy(d.pos)));
    return vec4(env.color * abs(dot(normal, normalize(vec3<f32>(0.30, 0.47, 0.80)))), 0.0);
}
